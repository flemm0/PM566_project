...
load.lib<-c("haven", "sjlabelled", "data.table", "tidyverse", "xml2", "rvest", "shiny", "plotly") #list of packages needed
...
```{r, message=FALSE}
df <- data.table::fread("./processed_data.csv")
```
...
server <- function(input, output, ...) {
  ...
  df %>%
    group_by(intake_day_cat, time_of_eating_occasion_hhmm) %>%
    summarise(m = mean(!! rlang::sym(x))) %>%
    plot_ly(x = ~time_of_eating_occasion_hhmm, y = ~m) %>%
