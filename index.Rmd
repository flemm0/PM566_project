...
server <- function(input, output, ...) {
  output$p <- renderPlotly({
    ...
    df %>%
      ...
      summarize(m = mean(!!rlang::sym(x)), sd = sd(!!rlang::sym(x))) %>%
      ...
      plot_ly(
        ...
        type = "bar") %>% 
      layout(barmode = "stack", xaxis = list(title = "Age Category (in years) and Gender"), 
             yaxis = list(title = paste("Average", input$ingredient, "Consumption (grams)", sep = " ")))
  })
}
...
df %>%
  select(racehispanic_origin_w_nh_asian,
         ...
         total_sugars_gm) %>%
  group_by(racehispanic_origin_w_nh_asian, age_category, gender) %>%
  summarize(m = mean(total_sugars_gm), sd = sd(total_sugars_gm)) %>%
  unite(age_gender, c(age_category, gender), sep = " ") %>%
  plot_ly(
    y = ~m, 
    x = ~factor(age_gender, levels = c(
    ...
    type = "bar") %>% 
  layout(barmode = "stack", xaxis = list(title = "Age Category (in years) and Gender"), 
         yaxis = list(title = paste("Average", "Sugar", "Consumption", sep = " ")))
...
to_plot %>%
  unite(age_gender, c(age_category, gender), sep = " ") %>%
  plot_ly(x = ~age_gender, y = ~m, color = ~racehispanic_origin_w_nh_asian, type = "bar") %>% layout(barmode = "stack")
