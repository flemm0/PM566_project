...
#sugar
unique(df[, .(sugar_consumption = sum(total_sugars_gm), racehispanic_origin_w_nh_asian, gender), by = "respondent_sequence_number"]) %>%
...
#saturated fa
unique(df[, .(sat_fa_consumption = sum(total_saturated_fatty_acids_gm), racehispanic_origin_w_nh_asian, gender), by = "respondent_sequence_number"]) %>%
...
quantile(df$total_sugars_gm, seq(0, 1, 0.05))["95%"]
...
```{r, fig.height = 8, fig.width = 12, fig.align = "center"}
quantile(df$total_saturated_fatty_acids_gm, seq(0, 1, 0.05)) #lets consider high amounts of sugar to be > 95th percentile 33.11
quantile(df$total_saturated_fatty_acids_gm, seq(0, 1, 0.05))["95%"]
