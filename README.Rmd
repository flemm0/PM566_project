...
```{r test to see if i can import table from website to encode variables instead, eval = FALSE, include = FALSE}
fs_test = copy(fs_d1)

table <- read_html("https://wwwn.cdc.gov/NCHS/nhanes/2017-2018/P_DR1IFF.htm#DR1DAY") %>%
  html_nodes(xpath = "/html/body/div[2]/div[4]/div[13]/table") %>%
  html_table() %>%
  as.data.frame() %>%
  select(Code.or.Value, Value.Description)

fs_test$combination_food_type[match(fs_test$combination_food_type, table$Code.or.Value)] <- table$Value.Description
